...
OSStatus MyEnqueueBuffer(AudioStreamer* myData)
{
	...
	// enqueue buffer
	AudioQueueBufferRef fillBuf = myData->audioQueueBuffer[myData->fillBufferIndex];
	fillBuf->mAudioDataByteSize = (unsigned int)myData->bytesFilled;
	
	if (myData->packetsFilled)
	{
		err = AudioQueueEnqueueBuffer(myData->audioQueue, fillBuf, (unsigned int)myData->packetsFilled, myData->packetDescs);
	}
	else
	{
